...
export class Database {
    ...
    `;
        ...
        this.db.run(`ALTER TABLE reviews ADD COLUMN app_id TEXT`, (err) => {
    ...
    async saveReviews(reviews) {
        if (this.inMemory) {
            ...
            const key = (r) => `${r.id}|${r.store}|${r.region}`;
            const map = new Map(this.memReviews.map(r => [key(r), r]));
    ...
    async getReviews(filters) {
        if (this.inMemory) {
            ...
            if (filters.appName)
                list = list.filter(r => r.appName === filters.appName);
            if (filters.appId)
                list = list.filter((r) => r.appId === filters.appId);
            if (filters.store)
                list = list.filter(r => r.store === filters.store);
        ...
        return new Promise((resolve, reject) => {
            ...
            this.db.get(countQuery, countParams, (err, row) => {
                if (err) {
                    reject(err);
                    return;
                }
